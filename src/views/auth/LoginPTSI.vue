			...
			<div class="right-section">
				...
				<div class="flex-direction-box">
					...
					<form @submit.prevent="checkAuth">
						...
						<router-link  class="forget mb-5" :to="{name: 'ForgotPasswordPTSI'}">
							<span style="color: #0D47A1;">{{ $t('login.forgot') }}</span>
							
						</router-link>
						<div>
							<v-btn type="submit" style="color: white;" block color="#0D47A1" class="mt-5">{{ $t('login.btn') }}</v-btn>
						</div>
						</div>
					</form>
					</div>
...
import AuthSidebar from "./AuthSidebarPTSI.vue"
...
.footer-container { /* Add a new class to your footer div for better targeting */
    display: flex;
    justify-content: space-between; /* Space out the two span elements */
    align-items: center; /* Vertically align items in the middle */
    padding: 10px 0; /* Add some vertical padding */
    font-size: 14px;
    color: #555; /* A subtle grey for text */
    border-top: 1px solid #eee; /* Optional: A subtle line above the footer */
    margin-top: auto; /* This is crucial for pushing the footer to the bottom in a flex column */
}
.footer-container strong {
    color: #333; /* Make the strong text a bit darker */
}
...
.vertical-center {
    ...
    margin: 0;
    position: relative; /* Change to relative or remove if not needed for vertical centering in the flex context */
    top: auto; /* Reset top */
    -ms-transform: none; /* Reset transform */
...
.flex-direction-box {
    ...
    flex-grow: 1; /* Allow this box to grow and take up available space */
    display: flex;
