...
export default {
    ...
    methods: {
        async getGraphData(filterData){
            ...
            try {
                const {data} = await this.$http.post('/dashboard/membershipTypesPeriodically', {
                    filterType: this.filterType,
                    data: filterData
                })
                // console.log('xxx', data.data)
                // const weekLables = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat','Sun']
                ...
                for (const month in data.data) {
                    // console.log('mm',month, data.data[month]);

                
                    if(data.data[month].length){
                        this.chartData.labels.push(month)
                        
                        data.data[month].forEach((item, i) => {
                            const colors = ['#FABE7A', '#F6866A','#59E6F6','#F40A34']
                            const x = _.find(this.chartData.datasets, {label: item.membershipType})
                            if(x){
                                // console.log('xf',x)
                                x.data.push(item.count)
                                // this.changedPerm.filter(item => item.id !== s.id)
                            }else{
                                var lx = {
                                ...
                                this.chartData.datasets.push(lx)
                            }
                        })
