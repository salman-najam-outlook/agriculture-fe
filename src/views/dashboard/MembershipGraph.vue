  ...
  <div>
    ...
    <v-card flat v-else>
      <v-toolbar flat class="pt-3">
        <v-toolbar-title class="mt-n3">{{ $t('dashboardSection.membershipValidity') }}</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-toolbar-items>
          <v-menu
            v-model="menu1"
  ...
  methods: {
    async getGraphData(filterData) {
      ...
      try {
        ...
        const gD = data.data;
        this.chartData.datasets[0].data.push(gD.lessThanMonth);
        this.chartData.datasets[0].data.push(gD.oneToSixMonths);
        this.chartData.datasets[0].data.push(gD.sixMonthsToOneYear);
        this.chartData.datasets[0].data.push(gD.moreThanYear);
        this.loading = false;
      } catch (err) {
        console.log("err", err);
        this.loading = false;
