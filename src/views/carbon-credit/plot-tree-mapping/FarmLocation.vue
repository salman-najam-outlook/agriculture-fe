        ...
        <v-row class="pt-4">
          ...
          <v-col cols="12" md="4">
            <v-card class="pa-6 pt-0" elevation="0">
              <h3 class="font-weight-bold mb-6">{{ $t('carbonCredit.plotTreeMapping.chooseFarmLocation') }}</h3>
              ...
              <v-form ref="locationForm" v-model="isFormValid">
                ...
                <div class="d-flex justify-space-between">
                  <v-btn outlined color="primary" class="mr-2" @click="onCancel" :disabled="loading"
                    style="min-width: 120px">
                    {{ $t('cancel') }}
                  </v-btn>
                  <v-btn color="primary" dark @click="onSave" :loading="loading" style="min-width: 120px">
                    {{ $t('save') }}
                  </v-btn>
                </div>
              </v-form>
    ...
    data() {
      return {
        ...
        breadcrumbs: [
          ...
          { text: 'Choose Farm Location', disabled: true, href: '#' },
        ],
        longitudeRules: [
    ...
    methods: {
      ...
      onSave() {
        ...
        const farmLocation = {
          ...
          address: this.search, // Assuming 'search' holds the address
          areaSize: this.areaSize,
          areaUnit: this.areaUnit
        };

        // Commit to Vuex store
        this.$store.dispatch('carbonCredit/setFarmLocation', farmLocation);
        ...
        try {
          ...
          service.getDetails({
            ...
            if (status === window.google.maps.places.PlacesServiceStatus.OK && place) {
              ...
              // Update the search input with the full address
        ...
        geocoder.geocode({ location }, (results, status) => {
          if (status === 'OK') {
            if (results[0]) {
              ...
              this.search = 'Address not found';
            }
          } else {
            console.error('Geocoder failed due to: ' + status);
            this.search = 'Address lookup failed';
          }
        });
      },
    }
