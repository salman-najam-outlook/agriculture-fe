      ...
      <div class="d-flex mb-8">
        ...
        <div>
          <div class="d-flex flex-row-reverse">
            ...
            <v-btn
              v-if="selectedSurvey.rows[0].survey.status !== 'Completed'"
              depressed
              color="primary"
              dark
              class="ml-3"
            ...
            <template v-if="selectedSurvey" v-slot:body="{ items }">
              <tbody>
                ...
                >
                  ...
                  <td class="text-center">{{ surveyScore(item) }} </td>
                  <td class="text-center">
                    <div
                      v-if="selectedItem ? selectedItem.id === item.id : false"
                    >
                      <v-tooltip bottom>
            ...
            <div class="d-flex justify-space-between mb-1">
              <div class="d-flex flex-column justify-space-between">
                ...
                <div>
                  ...
                  {{
                    survey.questionType === "dynamicGeoFence" ? survey.surveyQuestionsResponse.map((r) => r.geofence.geofenceName).join(", ") : ''
                  }}
                  {{
...
import AddNewSurveyDialog from "./SelectFarmDialog.vue";
  ...
  methods: {
    ...
    async getResponses() {
      ...
      await SurveyBuilderService.getResponse(this.$route.params.id, requestParams).then(
        (res) => {
          this.totalResponse = res?.data?.responsesCount;
    ...
    async downloadPdf() {
      this.startLoading()
      const responseDetailRows = document.getElementById("responseDetailRows");
      const titleDiv = document.getElementById('ttle-res')
      titleDiv.style.display = "block";
      const elRect = responseDetailRows.getBoundingClientRect();
      const width = 1280;
      await html2PDF(responseDetailRows, {
        html2canvas: {
          width: width,
          height: elRect.height + elRect.top + window.scrollY,
          windowWidth: width,
          x: -elRect.left / 2,
          y:-50
        },
        imageType: "image/jpeg",
        output: `${this.selectedSurvey.rows[0].survey.title} - ${this.userSurveyResponse.surveyEntity.users.fullName}.pdf`,
      });
