        ...
        <v-container fluid>
            ...
            <v-card elevation="0" class="px-5">
                <v-card-text>
                    ...
                    <h2 style="color:black; margin-top: 10px;">Risk Assessment & Mitigation
                    ...
                    <div v-if="reportRiskAssessmet && reportRiskAssessmet.farm == 'One for Each Farm'">
                        <div class="d-flex mt-5">
                            <h3 style="color:black">Production Places
                                <v-tooltip top color="00BD73" max-width="350">
                                    <template v-slot:activator="{ on, attrs }">
                                        <v-icon class="icon-with-background" style="color:#0EBF67" v-bind="attrs"
                                            v-on="on">
                                            mdi-alert-circle
                                            <!-- https://vuetifyjs.com/en/features/icon-fonts/#built-in-aliases -->
                                        </v-icon>
                        ...
                        <v-data-table :headers="placesHeaders" :items="productionPlaces" hide-default-footer
                            :server-items-length="totalProductionPlaces" loading-text="Production Places...">
                            <template v-slot:top="{}">
                                <div class="d-flex mt-5">
                                    <div style="width: 290px; margin-top: 22px;">
                                        ...
                                        <v-text-field prepend-inner-icon="mdi-magnify" outlined height="5px"
                                            ...
                                            @input="handleSearchByPharse" class="shrink" clearable></v-text-field>
                                    </div>
                                    <div class="ml-3" style="width: 290px;">
                                        <label for="text">{{ $t('Country') }}</label>
                                        <v-select :items="countries" v-model="country" item-text="name"
                                            item-value="name" dense outlined class="wdt" :placeholder="$t('Select')"
                                            @change="handleSearchByCountry"></v-select>
                                    </div>

                                    <div class="ml-3" style="width: 290px;">
                                        <label for="text">{{ $t('Supplier') }}</label>
                                        <!-- {{JSON.stringify(formattedSupplier)}} -->
                                        <v-select :items="formattedSupplier" v-model="suppplier" item-text="fullName"
                                            item-value="id" dense outlined class="wdt" :placeholder="$t('Select')"
                                            @change="handleSearchBySupplier"></v-select>
                                    </div>

                                    <div class="ml-3" style="width: 290px;">
                                        <label for="text">{{ $t('Risk Assessment Status') }}</label>
                                        <v-select :items="riskAssessmentStatusList" v-model="riskAssessmentStatus" dense
                                            outlined class="wdt" :placeholder="$t('Select')"
                                    ...
                                    <div class="d-flex" style="margin-top: 15px;">
                                        <div class="py-1 title">
                                            {{ options.page }} - {{ totalPages }} of {{ totalProductionPlaces }}
                                        </div>
                            ...
                            <template v-slot:body="{ items }">
                                <tbody>
                                    <tr v-for="(item, index) in items"
                                        :key="item.production_places_farm_id ? index + item.production_places_farm_id : index"
                                        style="position: relative; width: 0; height: 0">
                                        <td> {{ item.farm.farmName }} {{ item.farm.farmNumber }}</td>
                                        <td> {{ item.farm.userDdsAssoc.firstName }} {{ item.farm.userDdsAssoc.lastName }}
                                        </td>
                                        ...
                                        <td>
                                            <div class="d-flex">
                                                <v-btn small rounded min-width="175" depressed class="green-button"
                                                    height="34" v-if="item.risk_assessment == 'Approved'">
                                                    <!-- getModulePermittedActions.put -->
                                                    {{ $t('Approved') }}
                                                </v-btn>

                                                <v-btn small rounded min-width="175" depressed class="red-button"
                                                    height="34" v-if="item.risk_assessment == 'Rejected'">
                                                    <!-- getModulePermittedActions.put -->
                                                    {{ $t('Rejected') }}
                                                </v-btn>

                                                <v-btn small rounded min-width="175" depressed class="orange-button"
                                                    height="34" v-if="item.risk_assessment == 'Mitigation Required'">
                                                    <!-- getModulePermittedActions.put -->
                                                ...
                                                <img class="ml-1" src="/icons/todo-list.png"
                                                    ...
                                                    @click="handleProductionPlaceClick(item.production_places_farm_id)" />
                                                <v-icon style="cursor:pointer" class="ml-1 green-shadow"
                                                    @click="removeFarm(item.id)">mdi-trash-can</v-icon>
            ...
            <v-card elevation="0" class="px-5 mt-5">
                <v-card-text>
                    <v-form ref="surveyForm" @submit.prevent="addSurvey" autocomplete="off">
                        <v-row class="pl-3">
                            <v-col cols="6" class="mb-10">
                                    ...
                                    <div style="overflow: clip" :class="['dropZone', dragging ? 'dropZone-over' : '']"
                                        ...
                                        <div class="dropZone-info d-flex justify-center align-center" @dragover.prevent>
                                            ...
                                            <span class="dropZone-title">
                                                <v-icon class="blue-color" size="50"
                                                    color="gray">mdi-tray-arrow-up</v-icon>
                                            </span>
                                            <div class="dropZone-upload-limit-info">
                                                <h2 class="blue-color underline" color="primary"> Select file to Upload
                                                    the Data</h2>
                                                <p class="gray-color"> or drag & drop here </p>
                                            </div>
                                        </div>
                                ...
                                <div class="pdf-docs mt-10 d-flex" v-for="(docs, key) in documents" :key="key">
                                    <div class="docs-box mr-4">
                                        <div class="docs-preview">
                                            <div class="img-item">
                                                <div class="pdf-box">
                                                    <v-icon @click="removeFile(key)"> mdi-trash-can-outline </v-icon>
                                    ...
                                    <div class="text-box mr-4">
                                        <v-textarea v-model="docs.comment" placeholder="Comments" outlined></v-textarea>
                                    </div>

                                    <div class="expiry-box">
                                        <label class="expiration" for="name">Expiry Date </label>
                                        <v-menu v-model="docs.productExpirationMenu" :close-on-content-click="false"
                                            ...
                                            <template v-slot:activator="{ on, attrs }">
                                                <v-text-field append-icon="mdi-calendar" outlined dense readonly
                                                    ...
                                                    v-model="docs.expiryDate" placeholder="Select Date"></v-text-field>
                                            </template>
                                            <v-date-picker placeholder="Select Date" v-model="docs.expiryDate"
                        ...
                        <div class="survey mt-8">
                            <div class="ttl d-flex justify-space-between mb-8">
                                <h2> Manually fill out !!!!!!!!! <v-icon> mdi-mdi-info </v-icon> </h2>
                                <v-btn color="primary"> Download Assessment Form </v-btn>
                            </div>

                            <Survey :surveyee="survey" @questionResponses="handleQuestionResponses"> </Survey>
                        </div>
                        ...
                        <div class="btn-div d-flex flex-row-reverse">
                            <v-btn type="submit" color="primary" width="112" @click="submitAssessment()"> Submit
                            </v-btn>
...
export default {
    ...
    methods: {
        async getDiligenceDetail(diligenceId) {
            this.loading = true;
            DeforestationService.getDiligenceDetail(parseInt(diligenceId)).then(res => {
                const dueDiligence = res.data.getDiligenceDetail;

                const dummyRequiredAssessment = [
                    { "farm": "One for Each Farm", "type": "Existing Report", "assessment": "Farm Risk Assessment" },
                    { "farm": "One for All Farms", "type": "Existing Report", "assessment": "EUDR Deforestation Assessment" },
                    { "id": 5, "farm": "One for All Farms", "type": "Existing Report", "assessment": "Legality Information" },
                    { "id": 6, "farm": "One for All Farms", "type": "Existing Report", "assessment": "Compliance Assessment" }];
                dueDiligence.requiredAssessment = dummyRequiredAssessment;

                const riskAssessment = dueDiligence.requiredAssessment.find(assessment => (assessment.assessment == 'Farm Risk Assessment'))
        ...
        async getProductionPlaces(opt = null) {
            ...
            DeforestationService.getProductionPlaces(requestParams).then(res => {
        ...
        async uploadFiles(files) {
            ...
            if (invalidFiles.length > 0) {
                ...
                this.documents = validFiles.map(x => {
                    return {
                        file: x,
                        extension: x.name.split('.').pop().toLowerCase(),
                        name: x.name,
                        comment: '',
                        expiryDate: '',
                ...
                this.documents.forEach(async (item) => {
                    const result = await S3UploadService.singleUpload(item.file, 'risk-assessment-docs');
                    const s3Result = {
        ...
        findQuestionForSurvey(assessmentId) {
            DeforestationSurveyBuilderService.findQuestionForSurvey(parseInt(assessmentId)).then((res) => {
                const noOfSteps = res.data.findQuestionForSurvey.count;
                this.totalSurveyStep = noOfSteps;
                this.stepWiseSurveyQuestions = this.survey = res.data.findQuestionForSurvey.rows;
                this.survey = res.data.findQuestionForSurvey.rows[0].assessmentQuestions;
                this.stopLoading()
            })
        },
        ...
        submitSurveyAssessment(dueDiligenceId, assessmentId, userId) {
            ...
            if (this.isOneForEachFarm) {
                const index = this.surveyResponseWithProductionPlaceFarmId.findIndex(s => s.farmId === parseInt(this.selectedProductionPlaceFarmId))
                const productionPlaceFarmData = {
                    farmId: parseInt(this.selectedProductionPlace),
                    surveyResponses: this.surveyResponses
                }
                if (index === -1)
                    this.surveyResponseWithProductionPlaceFarmId.push(productionPlaceFarmData)
                else this.surveyResponseWithProductionPlaceFarmId[index] = productionPlaceFarmData
            }
            ...
            const payload = {
                ...
                userId,
                farmId: this.isOneForEachFarm ? parseInt(this.selectedProductionPlace) : null,
                surveyResponses: this.surveyResponses,
                status: 'COMPLETED'
            }
        ...
        submitAssessment() {
            this.submitSurveyAssessment(this.diligenceId, this.assessmentId, this.getCurrentUser.id,);
        },
        handleProductionPlaceClick(productionPlaceFarmId) {
            ...
            const isExisting = this.surveyResponseWithProductionPlaceFarmId.find(s => s.farmId === parseInt(productionPlaceFarmId))
    ...
    data() {
        return {
            ...
            diligenceId: this.$route.params.id || 2,
            search: "",
            farmStatus: 'success',
            farmStatusList: [],
            ...
            placesHeaders: [
                {
                    text: this.$t('Production Place & Farm ID'),
                    align: 'start',
                    sortable: true,
                    value: 'user',
                    class: "black--text",
                ...
                {
                    text: `${this.$t('Address of Production')}`,
                    value: 'farm',
                    sortable: true,
                    class: "black--text",
                    width: '150px'
