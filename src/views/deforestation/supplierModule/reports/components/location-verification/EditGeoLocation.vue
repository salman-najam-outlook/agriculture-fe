...
export default {
  ...
  methods: {
    ...
    updateGeolocation() {
      this.geolocation = {
        producerName:
          this.farmDetails?.farm?.userAssoc?.firstName ||
          "" + " " + this.farmDetails?.farm?.userAssoc?.lastName ||
          "",
        producerCountry: this.farmDetails?.farm?.userAssoc?.countryId || "",
        farm: {
    ...
    async createCircle(radius = 0) {
      console.log({ radius }, this.map, this.latitude, this.longitude);
    ...
    async save() {
      if (
        ...
        let payloadData = {
          ...
          dueDiligenceReportId: this.farmDetails.dueDiligenceReportId,
          farms: {
            farmName: this.geolocation.farm.farmName,
            area:
              parseFloat(this.area) ?? parseFloat(this.geolocation.farm.area),
            ...
            pointCoordinates:
              this.geofenceType === "POINT"
                ? {
                    ...
                    radius: this.geofenceRadius.toString() ?? "0",
                  }
                : {},
            coordinates:
