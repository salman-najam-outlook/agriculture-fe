    ...
    <v-dialog v-model="dialog" width="800" @click:outside="closeModal">
      <v-card rounded="0">
        <v-toolbar color="primary" class="white--text" flat>
          <v-toolbar-title class="font-weight-bold"> {{ $t("dueDiligence.addSignature") }}
          </v-toolbar-title>
          <v-spacer></v-spacer>
        ...
        <v-card-text class="pa-10 mb-5">
          <div class="signature-controls">
            <v-radio-group v-model="controlOpt" mandatory row>
              <v-radio label="Draw Signature" value="signature"></v-radio>
              <v-radio label="Upload Image" value="upload_image"></v-radio>
            </v-radio-group>
            <hr>
          </div>
          <div class="signatur" v-if="controlOpt == 'signature'">
            <h2 class="mt-4 mb-4 text-center"> Sign Here </h2>
            <div class="draw-signature">
              <template>
                <VueSignaturePad id="signature" :options="options" width="500px" height="400px"
                                 ref="signaturePad"/>
              </template>
            </div>
          </div>
          ...
          <div class="upload-signature mt-4" v-else>
            <div class="upload-document">
              <div style="overflow: clip" :class="['dropZone', dragging ? 'dropZone-over' : '']"
                   @dragenter="dragging = true" @dragleave="dragging = false" @drop="handleDrop($event)">
                <div class="dropZone-info d-flex justify-center align-center"
                  ...
                  <div class="dropZone-upload-limit-info">
                    <h2 class="blue-color underline" color="primary"> Select file to Upload the
                      Signature</h2>
            ...
            <p class="mt-3"> Important Note: Please Provide follwing format (PNG/JPEG) </p>

            <div class="docs-box mr-4" v-if="uploadedFileName">
              <div class="docs-preview">
                <div class="img-item">
                  <div class="pdf-box">
                    <v-icon @click="removeSignature()"> mdi-trash-can-outline</v-icon>
          ...
          <v-card-text class="black--text">
            <v-card-actions class="mt-10">
              ...
              <v-btn width="112" color="primary" type="submit" class="btn-11 btn-22" @click="handleSubmit">
                {{ $t('submit') }}
              </v-btn>
            </v-card-actions>
          </v-card-text>
  ...
  methods: {
    ...
    handleFileChange(event) {
      ...
      if (file && (file.type === 'image/png' || file.type === 'image/jpeg' || file.type === 'image/gif')) {
        ...
        this.fileError = 'Invalid file format. Please upload a PNG, JPEG, or GIF image.';
      }
    },
    ...
    async handleSubmit() {
      ...
      try {
        if (this.controlOpt === "upload_image" && this.uploadedFile) {
          const result = await S3UploadService.singleUpload(this.uploadedFile, "signatures");
          ...
          if (!isEmpty) {
            ...
            const result = await S3UploadService.bolbImageUpload(blob, "signatures");
            this.s3Location = result.fullPath;
            this.s3key = result.key;
            //Use  s3Location/ s3key  while submitting manual assessment form
