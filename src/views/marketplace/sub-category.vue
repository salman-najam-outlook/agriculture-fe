...
export default {
  ...
  watch: {
    'createdByAdmin'(newValue) {
      this.options.created_by_admin = newValue ? 1 : 0;
      this.fetchCategoriesData();
    },
  },
  ...
  computed: {
    ...
    headers() {
      const baseHeaders = [
        ...
        {
          text: this.$t("marketplace.category.products"),
  ...
  methods: {
    async fetchCategoriesData() {
      try {
        ...
        const response = await marketplaceService.getSubCategoriesByID(this.id, {
          ...this.options,
        ...
        this.parentCategory = this.items[0].category.name
        this.stopLoading();
      } catch (error) {
        this.stopLoading();
